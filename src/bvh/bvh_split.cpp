                                            ...
                                            BoundBox &right_bounds)
{
   ...
   * not to matter. */
  float3 point = pointcloud->points[prim_index];
  ...
  if (geom->type == Geometry::MESH) {
    ...
    for (int point_idx = 0; point_idx < pointcloud->num_points(); ++point_idx) {
      split_point_primitive(
          pointcloud, &object->tfm, point_idx, dim, pos, left_bounds, right_bounds);
    }
  }
}
                                      ...
                                      float pos)
{
  ...
  if (ref.prim_type() & PRIMITIVE_ALL_TRIANGLE) {
    ...
    PointCloud *pointcloud = static_cast<PointCloud *>(ob->geometry);
