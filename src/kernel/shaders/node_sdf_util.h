...
// Similar to fOpunion_round, but more lipschitz-y at acute angles
// (and less so at 90 degrees). Useful when fudging around too much
// by MediaMolecule, from Alex Evans' siggraph slides
float sdf_op_union_soft(float a, float b, float r)
{
...
{
  float h = safe_divide(max(k - abs(a - b), 0.0), k);
  return min(a, b) - h * h * h * k * (1.0 / 6.0);
}

float sdf_op_difference_smooth(float a, float b, float k)
{
  float h = clamp(0.5 - 0.5 * (b + a) / k, 0.0, 1.0);
  return mix(b, -a, h) + k * h * (1.0 - h);
}

float sdf_op_intersection_smooth(float a, float b, float k)
{
  float h = clamp(0.5 - 0.5 * (b - a) / k, 0.0, 1.0);
  return mix(b, a, h) + k * h * (1.0 - h);
}
