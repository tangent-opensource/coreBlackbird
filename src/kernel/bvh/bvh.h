...
#  ifdef __EMBREE__
  if (kernel_data.bvh.scene) {
    ...
    RTC_NAMESPACE::rtcIntersect1(kernel_data.bvh.scene, &rtc_ctx.context, &ray_hit);
...
#    ifdef __EMBREE__
  if (kernel_data.bvh.scene) {
    ...
    if (has_bvh) {
      RTC_NAMESPACE::RTCGeometry geom = RTC_NAMESPACE::rtcGetGeometry(kernel_data.bvh.scene, local_object * 2);
      if (geom) {
        float3 P = ray->P;
        float3 dir = ray->D;
        float3 idir = ray->D;
        ...
        rtc_ray.dir_y = dir.y;
        rtc_ray.dir_z = dir.z;
        auto scene = (RTC_NAMESPACE::RTCScene)RTC_NAMESPACE::rtcGetGeometryUserData(geom);
        kernel_assert(scene);
        if (scene) {
          RTC_NAMESPACE::rtcOccluded1(scene, &rtc_ctx.context, &rtc_ray);
        }
      }
    }
    else {
      RTC_NAMESPACE::rtcOccluded1(kernel_data.bvh.scene, &rtc_ctx.context, &rtc_ray);
    }
